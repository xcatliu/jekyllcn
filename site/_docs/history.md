...
- 允许能被转换的被 >= 1 转换器转换 ([#2704]({{ site.repository }}/issues/2704))
- 允许 Sass 文件在 Liquid 中被渲染, 但永不把它们放到布局中 ([#2733]({{ site.repository }}/issues/2733))
- 增加 `jekyll help` 命令 ([#2707]({{ site.repository }}/issues/2707))
- 用 `.scss` 作为 `site_template` 样式 ([#2667]({{ site.repository }}/issues/2667))
- 在默认的 front matter 中不要求 `scope` 键 ([#2659]({{ site.repository }}/issues/2659))
- 不再在站点模板的 `_config.yml` 中设置 `permalink: pretty` ([#2680]({{ site.repository }}/issues/2680))
- 重做站点模板以使用 Sass ([#2687]({{ site.repository }}/issues/2687))
- 当自动生成被禁用的时候通知用户 ([#2696]({{ site.repository }}/issues/2696))
- 允许在 include 标签的文件名参数中使用部分变量 ([#2693]({{ site.repository }}/issues/2693))
- 移动 `Time.parse` 实例到 Utils 方法 ([#2682]({{ site.repository }}/issues/2682))
- 忽略 `_posts` 目录中的子目录 ([#2705]({{ site.repository }}/issues/2705)) REVERTS ([#2633]({{ site.repository }}/issues/2633))
- Front Matter 默认类型应该总是复数形式 ([#2732]({{ site.repository }}/issues/2732))
- 把静态文件到作为 `StaticFile` 读取到 `collection.files` ([#2737]({{ site.repository }}/issues/2737))
- 增加 `sassify` 和 `scssify` Liquid 过滤器 ([#2739]({{ site.repository }}/issues/2739))
- 替换 `classifier` gem 为 `classifier-reborn` ([#2721]({{ site.repository }}/issues/2721))

### Bug Fixes
{: #bug-fixes-v2-3-0}

- 当多个转换器存在的时候, 只使用最后一个扩展名 ([#2722]({{ site.repository }}/issues/2722))
- 在 jsonify 过滤器中, 在调用 `#to_json` 之前先调用 `#to_liquid`  ([#2729]({{ site.repository }}/issues/2729))
- 在 `strftime` 中使用非填充配置以避免重复解析字符串 ([#2673]({{ site.repository }}/issues/2673))
- 替换已弃用的 Ruby 方法为非已弃用的 ([#2664]({{ site.repository }}/issues/2664))
- 捕获解析 Post 的 `date` front matter 错误并输出有用的错误消息 ([#2649]({{ site.repository }}/issues/2649))
- 允许在集合中使用静态文件 ([#2615]({{ site.repository }}/issues/2615))
- 修复 `Deprecator#gracefully_require` 错误消息中的错别字 ([#2694]({{ site.repository }}/issues/2694))
- 去除抢先加载 'classifier' gem. ([#2697]({{ site.repository }}/issues/2697))
- 当加载配置文件的时候使用大小写不敏感的方式检查文件扩展 ([#2718]({{ site.repository }}/issues/2718))
- 当读取文档的时候, 把 `encoding` 配置考虑在内 ([#2720]({{ site.repository }}/issues/2720))
- 基于 jekyll-watch clean-up 的重构 ([#2716]({{ site.repository }}/issues/2716))
- `Document#to_s` 应当只生成文档的内容 ([#2731]({{ site.repository }}/issues/2731))

### Development Fixes
{: #development-fixes-v2-3-0}

- 在 gem 中只包含库文件 ([#2671]({{ site.repository }}/issues/2671))
- 修复 `proof` 脚本中的 `git diff` 命令 ([#2672]({{ site.repository }}/issues/2672))
- 默认的 rake 任务变为多任务, 以并行运行测试 ([#2735]({{ site.repository }}/issues/2735))

### Site Enhancements
{: #site-enhancements-v2-3-0}

- 使用 Sass 和 Docs 集合 ([#2651]({{ site.repository }}/issues/2651))
- 增加 `latest_version.txt` 文件到站点 ([#2740]({{ site.repository }}/issues/2740))
- ?更模棱两可的 `page.content`. 但是更易懂 ([#2522]({{ site.repository }}/issues/2522))
- 合理化 front matter 这个词 (而不是 front-matter/frontmatter) ([#2674]({{ site.repository }}/issues/2674))
- 添加关于在 GitHub Pages 中不能修改源目录的说明 ([#2669]({{ site.repository }}/issues/2669))
- 修复 [#2669]({{ site.repository }}/issues/2669) 的链接为真正的 HTML. 哎呀 ([#2679]({{ site.repository }}/issues/2679))
- 增加 `jekyll-slim` 链接到第三方插件列表 ([#2689]({{ site.repository }}/issues/2689))
- 添加 Barry Clark's Smashing Magazine 教程到资源页面 ([#2688]({{ site.repository }}/issues/2688))
- 整理和更新默认配置设置 ([#2456]({{ site.repository }}/issues/2456))
- 修复配置文档中关于 Redcarpet 扩展中的缩进 ([#2717]({{ site.repository }}/issues/2717))
- 在默认配置列表中使用 YAML 中的 `null` 替换 `nil`  ([#2719]({{ site.repository }}/issues/2719))
- 修复持续集成文档中的错别字 ([#2708]({{ site.repository }}/issues/2708))
